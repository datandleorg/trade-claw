...
- **F&O Options**: pick **one underlying** and **session date**; **strike policy** (ATM default = nearest listed strike to spot, plus ITM/OTM steps) and optional **manual strike** override. **Always 1 lot**. Exits: **target**, **stop loss** (slider; env `FO_OPTION_STOP_LOSS_PCT`), or **EOD**. **Net P/L** after ₹/lot costs. Changing any parameter **saves a JSON snapshot** under `data/fo_options_runs/` (`FO_OPTIONS_RUNS_DIR` to override) with **full params, metrics, trade details, and underlying/option OHLC series** for offline analysis. Sidebar **Navigate → F&O Options**.
