...
3. **Breakout decision** (per scan key), **order**: always run **`envelope_breakout_on_last_bar`** first (**20-period EMA**, bandwidth from **`env_trading_params.mock_engine_envelope_decimal_per_side(underlying)`** — **index** keys use optional **`MOCK_AGENT_INDEX_ENVELOPE_PCT`** when set; else **`fno_envelope_decimal_per_side()`** / **`MOCK_AGENT_ENVELOPE_PCT`**; geometry matches `strategies._envelope_series`). Warmup, penetration, clear-break, strict filters, and confirm bar: [MOCK_ENGINE_BREAKOUT_RULES.md](MOCK_ENGINE_BREAKOUT_RULES.md). If that rule **does not** fire, the underlying is skipped (no LLM). If it **does** fire (**provisional signal**): when **`MOCK_LLM_ATTACH_UNDERLYING_CHART=1`** and the **assessment PNG** builds, a **vision LLM** **validates** the rule-based call (confirm or veto from the chart). On **veto** (`breakout=false`), that underlying is skipped for the tick. If the chart cannot be built or the vision **API throws**, the worker **falls back** to the deterministic signal and records **`vision_validation_status`** (`skipped_no_chart` / `fallback_error`). If attach is off or PNG is missing, the pipeline continues on rules only with **`skipped_no_chart`**. **`MOCK_LLM_BREAKOUT_FROM_CHART`** is **deprecated** and ignored by the graph.
...
3. Optional **prompt trace** (`MOCK_LLM_PROMPT_LOG_DIR`): when **`llm_flow_dir`** is set on state (unified run from `signal`), files go under **`…/<session_date>/<HHMM>/flow_<id>/strike/`** (`system.txt`, `human.txt`, optional `chart.png`, `meta.json`, `llm_output.json` / `llm_error.json`, optional `retry_*.txt`). Otherwise the legacy per-strike folder under **`…/<date>/<HHMM>/<UNDERLYING>_…/`** is used for this node only.
...
| `MOCK_LLM_BREAKOUT_FROM_CHART` | **Deprecated (ignored).** Signal always runs deterministic envelope first, then vision **validation** when attach + PNG succeed (see § Node `signal`). |
...
| `MOCK_LLM_PROMPT_LOG_DIR` | If set, each provisional signal creates **`…/<session_date>/<HHMM>/flow_<id>/`** with **`deterministic.json`**, **`vision/`** (validation LLM), **`candidates.json`**, **`strike/`** (strike LLM), **`outcome.json`** (execute). Streamlit **Mock AI engine → LLM trace** tab lists `flow_*` dirs with a date range filter. Use the **same path on worker and Streamlit** (e.g. Docker volume under `/app/data/…`). Legacy orphan strike-only folders may still exist if flow was disabled at signal time. |
