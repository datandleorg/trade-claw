...
def mock_engine_option_target_rupees_above_entry() -> float | None:
    """
    When ``MOCK_ENGINE_OPTION_TARGET_RUPEES`` is a positive number, mock engines that use
    :func:`mock_engine_option_target_price` store take-profit premium as **entry + this many rupees**
    (overrides percentage targets for that path).
    """
...
def mock_engine_option_target_price(entry: float) -> float:
    """Resolved take-profit premium (₹) after entry is known (fixed rupees or percent)."""
